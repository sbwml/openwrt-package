...
if nixio.fs.access("/etc/ssrpro/gfw_list.conf") then
	gfw_count = tonumber(luci.sys.exec("cat /etc/ssrpro/gfw_list.conf | wc -l")) / 2
end

if nixio.fs.access("/etc/ssrpro/ad.conf") then
	ad_count = tonumber(luci.sys.exec("cat /etc/ssrpro/ad.conf | wc -l"))
end

if nixio.fs.access("/etc/ssrpro/china_ssr.txt") then
	ip_count = tonumber(luci.sys.exec("cat /etc/ssrpro/china_ssr.txt | wc -l"))
end

if nixio.fs.access("/etc/ssrpro/netflixip.list") then
	nfip_count = tonumber(luci.sys.exec("cat /etc/ssrpro/netflixip.list | wc -l"))
end

if Process_list:find("udp.only.ssrpro.reudp") then
	reudp_run = 1
end

if Process_list:find("tcp.only.ssrpro.retcp") then
	redir_run = 1
end

if Process_list:find("tcp.udp.ssrpro.local") then
	sock5_run = 1
end

if Process_list:find("tcp.udp.ssrpro.retcp") then
	redir_run = 1
	reudp_run = 1
end

if Process_list:find("local.ssrpro.retcp") then
	redir_run = 1
	sock5_run = 1
end

if Process_list:find("local.udp.ssrpro.retcp") then
	reudp_run = 1
	redir_run = 1
	sock5_run = 1
end
...
if Process_list:find("ssrpro/bin/pdnsd") or (Process_list:find("ssrpro.dns") and Process_list:find("dns2socks.127.0.0.1.*127.0.0.1.5335")) then
	pdnsd_run = 1
end
...
if uci:get_first("ssrpro", 'global', 'pdnsd_enable', '0') ~= '0' then
...
if uci:get_first("ssrpro", 'global', 'pdnsd_enable', '0') == '1' then
	s = m:field(DummyValue, "cache", translate("Reset pdnsd cache"))
	s.template = "ssrpro/cache"
end

s = m:field(DummyValue, "check_port", translate("Check Server Port"))
s.template = "ssrpro/checkport"
s.value = translate("No Check")
