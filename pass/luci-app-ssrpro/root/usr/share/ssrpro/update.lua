...
local TMP_DNSMASQ_PATH = "/var/etc/dnsmasq-ssrpro.d"
local TMP_PATH = "/var/etc/ssrpro"
	...
	if sret == 0 then
		...
		if new_md5 == old_md5 then
			...
			if type == "gfw_data" or type == "ad_data" then
				luci.sys.call("/usr/share/ssrpro/gfw2ipset.sh")
			else
				luci.sys.call("/usr/share/ssrpro/chinaipset.sh " .. TMP_PATH .. "/china_ssr.txt")
			end
			if args then
...
if args then
	if args == "gfw_data" then
		update(uci:get_first("ssrpro", "global", "gfwlist_url"), "/etc/ssrpro/gfw_list.conf", args, TMP_DNSMASQ_PATH .. "/gfw_list.conf")
		os.exit(0)
	end
	if args == "ip_data" then
		update(uci:get_first("ssrpro", "global", "chnroute_url"), "/etc/ssrpro/china_ssr.txt", args, TMP_PATH .. "/china_ssr.txt")
		os.exit(0)
	end
	if args == "ad_data" then
		update(uci:get_first("ssrpro", "global", "adblock_url"), "/etc/ssrpro/ad.conf", args, TMP_DNSMASQ_PATH .. "/ad.conf")
		os.exit(0)
	end
	if args == "nfip_data" then
		update(uci:get_first("ssrpro", "global", "nfip_url"), "/etc/ssrpro/netflixip.list", args)
		os.exit(0)
	end
else
	log("正在更新【GFW列表】数据库")
	update(uci:get_first("ssrpro", "global", "gfwlist_url"), "/etc/ssrpro/gfw_list.conf", "gfw_data", TMP_DNSMASQ_PATH .. "/gfw_list.conf")
	log("正在更新【国内IP段】数据库")
	update(uci:get_first("ssrpro", "global", "chnroute_url"), "/etc/ssrpro/china_ssr.txt", "ip_data", TMP_PATH .. "/china_ssr.txt")
	if uci:get_first("ssrpro", "global", "adblock", "0") == "1" then
		log("正在更新【广告屏蔽】数据库")
		update(uci:get_first("ssrpro", "global", "adblock_url"), "/etc/ssrpro/ad.conf", "ad_data", TMP_DNSMASQ_PATH .. "/ad.conf")
	end
	-- log("正在更新【Netflix IP段】数据库")
	-- update(uci:get_first("ssrpro", "global", "nfip_url"), "/etc/ssrpro/netflixip.list", "nfip_data")
end
