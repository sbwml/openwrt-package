...
m = Map("ssrpro", translate("Servers subscription and manage"))
...
o.write = function()
	...
	uci:save("ssrpro")
	uci:commit("ssrpro")
	luci.http.redirect(luci.dispatcher.build_url("admin", "services", "ssrpro", "delete"))
	return
end
...
s.sortable = true
s.extedit = luci.dispatcher.build_url("admin", "services", "ssrpro", "servers", "%s")
function s.create(...)
...
o.template = "ssrpro/ping"
o.width = "10%"

local global_server = uci:get_first('ssrpro', 'global', 'global_server') 
...
node.write = function(self, section)
	uci:set("ssrpro", '@global[0]', 'global_server', section)
	uci:save("ssrpro")
	uci:commit("ssrpro")
	luci.http.redirect(luci.dispatcher.build_url("admin", "services", "ssrpro", "restart"))
end
...
m:append(Template("ssrpro/server_list"))
