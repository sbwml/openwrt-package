...
if nixio.fs.access('/etc/cssr/gfw_list.conf') then
    gfwmode = 1
end
...
if gfwmode == 1 then
    gfw_count = tonumber(sys.exec('cat /etc/cssr/gfw_list.conf | wc -l')) / 2
    if nixio.fs.access('/etc/cssr/ad.conf') then
        ad_count = tonumber(sys.exec('cat /etc/cssr/ad.conf | wc -l'))
    end
end

if nixio.fs.access('/etc/cssr/china_ssr.txt') then
    ip_count = sys.exec('cat /etc/cssr/china_ssr.txt | wc -l')
end
